...
func main() {
	...
	v1 := router.Group("/api/news/v1")
	{
		...
		v1.GET("/hcfd", func(c *gin.Context) {
			...
			news[17] = LoadRSS("指傳媒", "https://feed.janicek.co/filter?url=http%3A%2F%2Fwww.fingermedia.tw%3Ffeed%3Drss2&include="+includeText)
			news[18] = LoadRSS("台灣好報 地方新聞", "https://feed.janicek.co/filter?url=http%3A%2F%2Ffeeds.feedburner.com%2Fnewstaiwan&include="+includeText)
