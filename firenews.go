...
func CleanupElements(elements []RssItem) []RssItem {
	keywords := "關鍵字搜尋搜尋|行善|廟|寺|地震.*局勢|局勢.*地震|地震.*經濟|經濟.*地震|收成|收成|價格|貸款|保險|價揚|治安|人壽|訂單|投資|火燒心|daily|產值|暴跌|價高|價低|量多|量少|民調|希望|拉麵|豪華|救護車模型|演技|綠能產業|冷門市場|人事地震|事業|表特|品牌|保險|鍋物"
	rp := regexp.MustCompile(keywords)
...
func main() {
	...
	v1 := router.Group("/api/news/v1")
	{
		...
		v1.GET("/drought", func(c *gin.Context) {
			includeText := "竹市.*乾旱|乾旱.*竹市|竹市.*缺水|缺水.*竹市|竹市.*旱災|旱災.*竹市|竹市.*停水|停水.*竹市|竹市.*限水|限水.*竹市|竹市.*水情|水情.*竹市|竹市.*旱季|旱季.*竹市|竹市.*供水|供水.*竹市|竹市.*蓄水|蓄水.*竹市"
			feeds := map[string]string{
