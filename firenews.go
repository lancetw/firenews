...
func main() {
	...
	if os.Getenv("GIN_MODE") == "release" {
		filterAPIPoint = "/firenews/api/util/v1/"
	} else {
	...
	v1 := router.Group("/api/news/v1")
	{
		v1.GET("/main", func(c *gin.Context) {
			...
			news[5] = LoadRSS("蘋果日報社會版", filterAPIPoint+"filter?url=http%3A%2F%2Fwww.appledaily.com.tw%2Frss%2Fcreate%2Fkind%2Frnews%2Ftype%2F102&include="+includeText)
			news[6] = LoadRSS("自由時報社會版", filterAPIPoint+"filter?url=http%3A%2F%2Fnews.ltn.com.tw%2Frss%2Fsociety.xml&include="+includeText)
			news[7] = LoadRSS("聯合新聞社會版", filterAPIPoint+"filter?url=http%3A%2F%2Fudn.com%2Fudnrss%2Fsocial.xml&include="+includeText)
			news[8] = LoadRSS("中國時報社會版", filterAPIPoint+"filter?url=http%3A%2F%2Fwww.chinatimes.com%2Frss%2Frealtimenews-society.xml&include="+includeText)
			news[9] = LoadRSS("蘋果日報國際版", filterAPIPoint+"filter?url=http%3A%2F%2Fwww.appledaily.com.tw%2Frss%2Fcreate%2Fkind%2Frnews%2Ftype%2F103&include="+includeText)
			news[10] = LoadRSS("自由時報國際版", filterAPIPoint+"filter?url=http%3A%2F%2Fnews.ltn.com.tw%2Frss%2Fworld.xml&include="+includeText)
			news[11] = LoadRSS("聯合新聞國際版", filterAPIPoint+"filter?url=http%3A%2F%2Fudn.com%2Fudnrss%2FBREAKINGNEWS4.xml&include="+includeText)
			news[12] = LoadRSS("中國時報國際版", filterAPIPoint+"filter?url=http%3A%2F%2Fwww.chinatimes.com%2Frss%2Frealtimenews-international.xml&include="+includeText)
			news[13] = LoadRSS("民眾日報", filterAPIPoint+"filter?url=http%3A%2F%2Fwww.mypeople.tw%2Frss%2F&include="+includeText)
			news[14] = LoadRSS("台灣新生報 地方綜合", filterAPIPoint+"filter?url=http%3A%2F%2Ffeeds.feedburner.com%2Ftssdnews&include="+includeText)
